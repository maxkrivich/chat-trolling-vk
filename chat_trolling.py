...
TODO: 
	...
	[+] mode 1 multi trolling top 10 dialogs
	[-] multithread trolling for mode1
'''
import sys
import json as j 
import time
...
def print_table(table):
	...
	for line in table:
		print('\t'+'-' * (len('| ' + ' | '.join('{:{}}'.format(x, col_width[i]) for i, x in enumerate(line)) + ' | ')-1))
		print('\t'+'| ' + ' | '.join('{:{}}'.format(x, col_width[i]) for i, x in enumerate(line)) + ' | ')
	print('\t'+'-' * (len('| ' + ' | '.join('{:{}}'.format(x, col_width[i]) for i, x in enumerate(table[-1])) + ' | ')-1))
...
def mode1(mail, password, n=10):
	...
	if internet_on():
		...
		print()
		while True:
			if v.send_typing_status(arr[i]):
				cnt += 1
				t = str(datetime.datetime.now())[11:]
			sys.stdout.write('\rtarget: {} requests: {} last success: {}'.format(result[arr[i]], cnt, t))
			sys.stdout.flush()
			time.sleep(0.3)
			i = (i + 1) % len(arr)
	else:
...
def header():
	print('\n\tChatTrolling v2.0.1 implementation by @maxkrivich\n')
	print(line)

def parse_args(args):
	email, password = '', ''
	usage = '\nUsage:\n\tchat_prolling.py -e <email> -p <password> -m <mode>\n'
	if len(args) == 0:
		print(usage)
		sys.exit(-1)
	try:
		opts, args = getopt.getopt(args, 'he:p:m:',['help','email=','password=','mode='])
...
def main(argv):
	...
	if mode == 0:
		...
		mode1(mail=email, password=password)
